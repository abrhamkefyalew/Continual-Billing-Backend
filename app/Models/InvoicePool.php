...
class InvoicePool extends Model
{
    ...
    //
    //
    // InvoicePool payment status constants (i.e. for the column status)
    ...
    /**
     * central list of Allowed InvoicePool Payment Statuses (status) as a method
     * 
     */
    ...
    {
        return [
            'status.in' => 'INVALID Invoice Pool Payment Status (status). Allowed values are: ' . implode(', ', self::allowedTypes()) . '.',  // this is just returning ERROR message,  SO using = [ implode(', ', self::allowedTypes()) ] here is NOT a Problem.
        ];
    }
